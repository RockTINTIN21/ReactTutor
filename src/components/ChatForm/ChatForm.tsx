...
function ChatForm({layoutState,setLayoutState,mainContentRef,mainContentWidth,setRef,userData}:ChatFormType) {
	...
	useEffect(() => {
		if(formControlChatRef.current && mainContentRef.current){
			...
			chatFormWrapperRef.current.style.width = `${mainContentRef.current!.offsetWidth}px`;
	...
	}, [clientWindowSize,mainContentWidth,userData.isNewChat]);
